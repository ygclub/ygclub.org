...
class jswizardmodel {
	...
	function preview_content($value){
		...
		$sql="select d.title,d.cid,d.author,d.did,d.time,d.summary from ".DB_TABLEPRE."doc as d left join ".DB_TABLEPRE."categorylink as c on d.did=c.did ";
		if($doc['js_type']!=4){
			$sql=$sql." left join ".DB_TABLEPRE."focus as f on d.did=f.did";
		}
		$sql=$sql." where 1=1 ";
		if($cid!=''){
			$sql=$sql." and c.cid in (".$cid.")";
		}
		if(0==$doc['js_type']){
		...
		$order=array();
		if($doc['js_type']!=4){
			$sql=$sql." and f.type=".$doc['js_type'];
		}
		$sql=$sql." group by d.title ";
	...
	//	echo $sql;
		...
		while($preview=$this->db->fetch_array($query)){
			...
			if($js_category!==false){
				$category=$this->get_doc_cid($preview['did']);
