...
//下面的内容是模板修改新加入内容。--------------------------------------------------------------------------------------------------------
	...
	function dogetconfig(){
		...
		if($inc_data){
			...
			if(!file_exists(HDWIKI_ROOT.'/block/'.$GLOBALS['theme']."/$block/{$fun}_inc.htm")){
				$tplfile="file://block/default/$block/{$fun}_inc";
			}else{
				$tplfile='file://block/' .$GLOBALS['theme']."/$block/{$fun}_inc";
			}
		...
		//这个地方用了xml的格式返回数据。在js端完成了赋值工作，好处是模板的*.inc.htm配置文件，不用关心数据赋值,PHP程序端也不用关心，方便了用户。
	...
	function dosavetemp(){
		...
		if(file_exists($this->tempfile)){//如果文件存在，则将$tmp变量加载进来。
