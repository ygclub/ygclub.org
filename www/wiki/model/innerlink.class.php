	...
	*/
		...
		foreach($this->titles as $title=>$row){
			if($row[0]){
				...
				if($setting['seo_type_doc'] && $setting['seo_type']){
					//使用title的rewrite
					$content = str_replace($row[1], $setting['site_url'].'/wiki/'.$title, $content);
				}else if($setting['seo_type']){
					//使用did的rewrite
					$content = str_replace($row[1], $setting['site_url'].'/doc-view-'.$row[0].$setting['seo_suffix'], $content);
				}else{
					//不支持rewrite
					$content = str_replace($row[1], $setting['site_url'].'/'.$setting['seo_prefix'].'doc-view-'.$row[0].$setting['seo_suffix'], $content);
				}
			}else{
				//内链不存在
	...
	function save($did, &$titlelist){
		...
		if(!empty($rows)){
			$rows2=array();
			foreach($rows as $i=> $row){
				$rows2[ urlencode($row['title']) ] = $row['did'];
			}
		}
		$rows = $rows2;
		...
		foreach($titlelist as $key=>$title){
			...
			$titleid=isset($rows[$title2]) ? $rows[$title2]:0;
