...
class control extends base{
	...
	function donoticemail() {
		if(!isset($this->post['submit'])) {
			if(isset($this->setting['noticemail'])) {
...
?>";
	...
	function dodocset(){
		if(!isset($this->post['docsubmit'])){
			$list = array('sandbox_id','verify_doc','comments','filter_external','base_createdoc', 'base_isreferences');
			foreach($list as $value){
				$basecfginfo[$value] = $this->setting[$value];
			}
			$this->view->assign('basecfginfo',$basecfginfo);
