...
/*
	...
	$Id: noforumdisplay.php 17459 2008-12-24 01:20:28Z zhaoxiongfei $
*/

define('CURSCRIPT', 'noforumdisplay');
...
} elseif(empty($forum['fid']) || $forum['fid'] == 0) {
...
if($forum['type'] == 'forum') {
	...
	$navigation = '&raquo; <a href="noforumdisplay.php?fid='.$forum['fup'].'">'.$forumup.'</a> &raquo; '.$forum['name'];
...
if($forum['password']) {
	if($action == 'pwverify') {
		if($pw != $forum['password']) {
			...
			showmessage('forum_passwd_correct', "noforumdisplay.php?fid=$fid");
		}
	} elseif($forum['password'] != $_DCOOKIE['fidpw'.$fid]) {
		include template('noforumdisplay_passwd');
		exit();
	}
}
...
$moderatedby = moddisplay($forum['moderators'], 'noforumdisplay');
	...
	if($sub['type'] == 'sub' && $sub['fup'] == $fid && (!$hideprivate || !$sub['viewperm'] || forumperm($sub['viewperm']) || strstr($sub['users'], "\t$discuz_uid\t"))) {
		...
		$sql = $accessmasks ? "SELECT f.fid, f.fup, f.type, f.name, f.threads, f.posts, f.todayposts, f.lastpost, ff.description, ff.moderators, ff.icon, ff.viewperm, a.allowview FROM {$tablepre}forums f
					...
					: "SELECT f.fid, f.fup, f.type, f.name, f.threads, f.posts, f.todayposts, f.lastpost, ff.description, ff.moderators, ff.icon, ff.viewperm FROM {$tablepre}forums f
						...
						WHERE f.status>0 AND f.type='sub' ORDER BY f.displayorder";
		$query = $sdb->query($sql);
...
if($forum['simple'] & 1) {
	...
	include template('noforumdisplay_simple');
	exit();
}
...
isset($orderby) && in_array($orderby, array('lastpost', 'dateline', 'replies', 'views')) ? $noforumdisplayadd .= "&amp;orderby=$orderby" : $orderby = $_DCACHE['forums'][$fid]['orderby'] ? $_DCACHE['forums'][$fid]['orderby'] : 'lastpost';
isset($ascdesc) && in_array($ascdesc, array('ASC', 'DESC')) ? $noforumdisplayadd .= "&amp;ascdesc=$ascdesc" : $ascdesc = $_DCACHE['forums'][$fid]['ascdesc'] ? $_DCACHE['forums'][$fid]['ascdesc'] : 'DESC';
...
if(empty($filter)) {
	...
	$threadcount = $sdb->result_first("SELECT COUNT(*) FROM {$tablepre}threads WHERE $filteradd AND displayorder>='0'");
}
...
$multipage = multi($threadcount, $tpp, $page, "noforumdisplay.php?fid=$fid$noforumdisplayadd", $threadmaxpages);
$extra = rawurlencode("page=$page$noforumdisplayadd");
...
while(($querysticky && $thread = $sdb->fetch_array($querysticky)) || ($query && $thread = $sdb->fetch_array($query)) || ($queryfloat && $thread = $sdb->fetch_array($queryfloat))) {
	...
	$thread['typeid'] = $thread['typeid'] && !empty($forum['threadtypes']['prefix']) && isset($forum['threadtypes']['types'][$thread['typeid']]) ?
		'<em>[<a href="noforumdisplay.php?fid='.$fid.'&amp;filter=type&amp;typeid='.$thread['typeid'].'">'.$forum['threadtypes']['types'][$thread['typeid']].'</a>]</em>' : '';

	$thread['sortid'] = $thread['sortid'] && !empty($forum['threadsorts']['prefix']) && isset($forum['threadsorts']['types'][$thread['sortid']]) ?
		'<em>[<a href="noforumdisplay.php?fid='.$fid.'&amp;filter=sort&amp;sortid='.$thread['sortid'].'">'.$forum['threadsorts']['types'][$thread['sortid']].'</a>]</em>' : '';
...
include template('noforumdisplay');
