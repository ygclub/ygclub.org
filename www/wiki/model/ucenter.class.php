...
class ucentermodel {
	...
	function create_feed($doc,$did){
		...
		if(false!==strpos($isimg,"http://")){
			...
			$img=empty($isimg)?"":$this->base->setting[site_url]."/".$isimg;
		}
		$doc[did]=$did;
		...
		if(@in_array("create",$feed)){
			...
			$feed["title_data"] = array(
				"title" => "<a href=\\\"{$this->base->setting[site_url]}/{$this->base->setting[seo_prefix]}doc-view-$doc[did]{$this->base->setting[seo_suffix]}\\\">$doc[title]</a>",
				"author" => "<a href=\\\"space.php?uid={$uid}\\\">$this->user[username]</a>",
				"app"=>$this->base->setting["site_name"]
			);
			$feed["body_data"] = array(
				"subject"=> "<a href=\\\"{$this->base->setting[site_url]}/{$this->base->setting[seo_prefix]}doc-view-$doc[did]{$this->base->setting[seo_suffix]}\\\">$doc[title]</a>",
				"message"=> $doc["summary"]
			);
			$feed["images"][]= array("url" => "{$img}", "link" => "{$this->base->setting[site_url]}/{$this->base->setting[seo_prefix]}doc-view-$doc[did]{$this->base->setting[seo_suffix]}");
			$this->postfeed($feed);
		}
