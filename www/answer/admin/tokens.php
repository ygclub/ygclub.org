            ...
            foreach ($tokenfieldorder as $tokenfieldname)
            {
                ...
                if ($tokenfieldname=='tid')
                {
                    ...
                    if ($brow['completed'] != "N" && $brow['completed']!="" && $surveyprivate == "N"  && $thissurvey['active']=='Y')
                    {
                        ...
                        if  ($id)
                        {
                            $tokenoutput .= "fdsfdsa<input type='image' src='{$imageurl}/token_viewanswer.png' style='height: 16; width: 16px;' onclick=\"window.open('$scriptname?action=browse&amp;sid=$surveyid&amp;subaction=id&amp;id=$id', '_top')\" title='"
