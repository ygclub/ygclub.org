			...
			if($error=='OK'){
				...
				if($uid){
					...
					if('0' !== $this->setting['send_welcome']) {
						...
						$mail_content = str_replace('_LINK_', "<a href='{$this->setting['site_url']}'>{$this->setting['site_url']}</a>", $mail_content);
...
$this->message($error,'BACK',0);
	...
	function doinvite() {
		...
		$invite_url = $this->setting['site_url'].'/'.$this->view->url('user-register-'.$invite_code);
