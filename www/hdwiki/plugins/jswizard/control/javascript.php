...
class control extends base{
	...
	function dodefault(){
		if(trim($this->plugin[jswizard][vars][jsrefdomains])){
			$jsrefdomains=trim($this->plugin[jswizard][vars][jsrefdomains]);
		}
		...
		if(0==$jsstatus){
			...
			if(!$isvalid){
				...
				$previewcon=array();
				if(1==$this->get[2]){
					$previewcon['value']=$_ENV['jswizard']->preview_content($jslist[value]);
				}elseif(2==$this->get[2]){
					$previewcon['value']=$_ENV['jswizard']->preview_user_content($jslist[value]);
				}elseif(3==$this->get[2]){
					$previewcon['value']=$_ENV['jswizard']->preview_cate_content($jslist[value]);
				}elseif(5==$this->get[2]){
					$previewcon['value']=$_ENV['jswizard']->preview_tag_content($jslist[value]);
				}elseif(4==$this->get[2]){
					$previewcon['value']=$_ENV['jswizard']->preview_search_content($jslist[value]);
				}
				$this->cache->writecache('jswizard_'.$this->get[3],$previewcon);
			}
			$preview=$this->cache->getcache('jswizard_'.$this->get[3],$cachetime);
			$preview=stripslashes($preview['value']);
