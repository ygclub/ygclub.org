...
class ucusercontrol extends base {
	...
	function onsynlogin() {
		...
		if($this->app['synlogin']) {
			if($this->user = $_ENV['ucuser']->get_user_by_uid($uid)) {
				$synstr = '';
	...
	function onregister() {
		...
		$uid = $_ENV['ucuser']->add_user($username, $password, $email, 0, $questionid, $answer, $regip);
		return $uid;
	}
	...
	function onedit() {
		...
		$status = $_ENV['ucuser']->edit_user($username, $oldpw, $newpw, $email, $ignoreoldpw, $questionid, $answer);
	...
	function onlogin() {
		...
		$answer = $this->input('answer');
		if($isuid == 1) {
			$user = $_ENV['ucuser']->get_user_by_uid($username);
		} elseif($isuid == 2) {
			$user = $_ENV['ucuser']->get_user_by_email($username);
		} else {
			$user = $_ENV['ucuser']->get_user_by_username($username);
		}
		...
		if(empty($user)) {
			...
			$status = -2;
		} elseif($checkques && $user['secques'] != '' && $user['secques'] != $_ENV['ucuser']->quescrypt($questionid, $answer)) {
			$status = -3;
		} else {
			$status = $user['uid'];
		}
		$merge = $status != -1 && !$isuid && $_ENV['ucuser']->check_mergeuser($username) ? 1 : 0;
	...
	function onget_user() {
		...
		$username = $this->input('username');
		if(!$this->input('isuid')) {
			$status = $_ENV['ucuser']->get_user_by_username($username);
		} else {
			$status = $_ENV['ucuser']->get_user_by_uid($username);
		}
        //print_r($status);
		if($status) {
	...
	function ondelete() {
		$this->init_input();
		$uid = $this->input('uid');
		return $_ENV['ucuser']->delete_user($uid);
	}
	...
	function onmerge() {
		...
		$uid = $_ENV['ucuser']->add_user($newusername, $password, $email, $uid);
	...
	function _check_username($username) {
		...
		if(!$_ENV['ucuser']->check_username($username)) {
			return UC_USER_CHECK_USERNAME_FAILED;
		} elseif(!$_ENV['ucuser']->check_usernamecensor($username)) {
			return UC_USER_USERNAME_BADWORD;
		} elseif($_ENV['ucuser']->check_usernameexists($username)) {
	...
	function _check_email($email, $username = '') {
		...
		if(!$_ENV['ucuser']->check_emailformat($email)) {
			return UC_USER_EMAIL_FORMAT_ILLEGAL;
		} elseif(!$_ENV['ucuser']->check_emailaccess($email)) {
			return UC_USER_EMAIL_ACCESS_ILLEGAL;
		} elseif(!$this->settings['doublee'] && $_ENV['ucuser']->check_emailexists($email, $username)) {
