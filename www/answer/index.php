...
function SendSubmitNotifications()
{
    ...
    if (!empty($thissurvey['emailresponseto']))
    {
        ...
        unset ($aFullResponseTable['refurl']);
        unset ($aFullResponseTable['ipaddr']);
        ...
        $sSubjectAppend = array('name'=>'', 'team'=>'');
    
        foreach ($aFullResponseTable as $sFieldname=>$fname)
        {
            if ($sFieldname == '85827X19X506' && validate_email($fname[2]))
            {
                $aEmailResponseTo[] = $fname[2];
            }
            if ($sFieldname == '85827X19X511' || $sFieldname == '85827X19X505' || $sFieldname == '85827X19X661')
            {
                $fname[2] = preg_replace("/\[[^[.]*\]$/", "", $fname[2]);
            }
            if ($sFieldname == '85827X19X511')
            {
                $sSubjectAppend['team'] = trim($fname[2]);
            }
            if ($sFieldname == '85827X19X504')
            {
                $sSubjectAppend['name'] = ' -- ' . $fname[2];
            }
            if (substr($sFieldname,0,4)=='gid_')
            {
                // Do not print group info. modified  by leeyupeng @ 2012-11-04
    ...
    if (count($aEmailNotificationTo)>0)
    {
        ...
        $oMail = new PHPMailer;
        // Modified by leeyupeng @2012-11-04 17:34
        //foreach ($aEmailNotificationTo as $sRecipient)
        //{
            if (!SendEmailMessage($oMail, $sMessage, $sSubject, $aEmailNotificationTo, $sFrom, $sitename, true, getBounceEmail($surveyid)))
            {
                if ($debug>0)
                {
    ...
    if (count($aEmailResponseTo)>0)
    {
        ...
        $sSubject=$sSubjectAppend['team'] . templatereplace($thissurvey['email_admin_responses_subj'],$aReplacementVars) . $sSubjectAppend['name'];
        $mail = new PHPMailer;
        //foreach ($aEmailResponseTo as $sRecipient)
        //{
            if (!SendEmailMessage($mail,$sMessage, $sSubject, $aEmailResponseTo, $sFrom, $sitename, true, getBounceEmail($surveyid)))
            {
                if ($debug>0)
                {
                    echo '<br />Email could not be sent. Reason: '.$maildebug.'<br/>';
